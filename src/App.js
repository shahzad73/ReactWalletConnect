...
function App() {
  ...
  const handleConnectTokenButtonClick = async () => {
      ...
      const NameContract = new provider.eth.Contract(smartContractABI, tokenAddress);
  ...
  const deployContractTransaction = async () => {     
    ...
    // var gasPrice = await provider.eth.getGasPrice();

    await provider.eth.getTransactionCount(connectedAccount, 'pending').then(obj=> {

          const encodedParameters = provider.eth.abi.encodeParameters(
    ...
    <header className="App-header">
      ...
      {!connectedAccount ? (
        ...
        <div>
                ...
                <p>Wallet Connected to <br /> {connectedAccount}
                <br />
